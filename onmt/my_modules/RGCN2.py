...
class RGCN2Layer(nn.Module):
    ...
    def __init__(self,
                 ...
                 use_gates=True,
                 use_glus=False):
        super(RGCN2Layer, self).__init__()
        ...
        if in_arcs:
            self.V_in = Parameter(torch.Tensor(self.num_units, self.num_units))
        ...
        if out_arcs:
            self.V_out = Parameter(torch.Tensor(self.num_units, self.num_units))
