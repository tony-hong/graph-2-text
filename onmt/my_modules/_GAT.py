...
class GATLayer(nn.Module):
    """ Graph isomorphism neural network encoder.
    ...
    def forward(self, src, lengths=None, arc_tensor_in=None, arc_tensor_out=None,
        ...
        max_degree = 1
        input_ = encoder_outputs.view((batch_size * seq_len, self.num_inputs))  # [b* t, h_input]

        if self.in_arcs:
            input_in = torch.mm(input_, self.V_in)  # [b* t, h_input] * [h_input, h] = [b*t, h]
            ...
            degr = int(first_in.size()[0] / batch_size / seq_len) # number of in degree
            ...
            if self.use_gates:
                ...
                input_in_gate = torch.mm(input_, self.V_in_gate)  # [b* t, h_input] * [h_input, 1] = [b*t, 1]
                ...
                in_gate = (first_in_gate + second_in_gate).view((batch_size, seq_len, degr)) # [b* t* degr, 1]
        ...
        if self.in_arcs and self.out_arcs:
            ...
            if self.use_gates:
                potentials_gate = torch.cat((in_gate, out_gate, same_input_gate), dim=2)  # [b, t,  mxdeg, 1]
            mask_soft = torch.cat((mask_in, mask_out, mask_loop), dim=1)  # [b* t, mxdeg]
        elif self.out_arcs:
            potentials = torch.cat((out_, same_input), dim=2)  # [b, t,  mxdeg, h]
            if self.use_gates:
                potentials_gate = torch.cat((out_gate, same_input_gate), dim=2)  # [b, t,  mxdeg, 1]
            mask_soft = torch.cat((mask_out, mask_loop), dim=1)  # [b* t, mxdeg]
        elif self.in_arcs:
            potentials = torch.cat((in_, same_input), dim=2)  # [b, t,  mxdeg, h]
            if self.use_gates:
                potentials_gate = torch.cat((in_gate, same_input_gate), dim=2)  # [b, t,  mxdeg, 1]
            mask_soft = torch.cat((mask_in, mask_loop), dim=1)  # [b* t, mxdeg]
        else:
            potentials = same_input  # [b, t,  mxdeg, h]
            if self.use_gates:
                potentials_gate = same_input_gate # [b, t,  mxdeg, 1]
            mask_soft = mask_loop  # [b* t, mxdeg]

        potentials_resh = potentials.view((batch_size * seq_len,
                                           max_degree, self.num_units,))  # [b * t, mxdeg, h]
