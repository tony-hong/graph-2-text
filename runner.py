...
def translate(model_path,output_path,gpu,data_part,corpus,max_length,block_ngram_repeat): 
    translate_cmd = 'python3 translate.py \
                -src processed_corpus/SR19_T2_delex/T2-{3}_{4}-ud-{3}_DEEP.delex-src-nodes.txt \
                -tgt processed_corpus/SR19_T2_delex/T2-{3}_{4}-ud-{3}_DEEP.output.dat \
                -src_label processed_corpus/SR19_T2_delex/T2-{3}_{4}-ud-{3}_DEEP.delex-src-labels.txt \
                -src_node1 processed_corpus/SR19_T2_delex/T2-{3}_{4}-ud-{3}_DEEP.delex-src-node1.txt \
                -src_node2 processed_corpus/SR19_T2_delex/T2-{3}_{4}-ud-{3}_DEEP.delex-src-node2.txt \
                -replace_unk \
                -verbose -dynamic_dict -batch_size 1 \
                -max_length {5} \
                -block_ngram_repeat {6} \
                -model save/{0} \
                -output outputs/{1} \
                -gpu {2} \
                -data_type gcn'.format(model_path,output_path,gpu,data_part,corpus,max_length,block_ngram_repeat) 
    exe(translate_cmd)
...
def train(configs):
    ...
    train_cmd = 'python3 train.py \
        ...
        configs['data'],
        configs['save_model'],
        configs['decoder_layers'],
        configs['epoch'],
        configs['learning_rate'],
        configs['gcn_num_layers'],
        configs['gcn_num_labels'],
        configs['gcn_residual'],
        configs['lambda_coverage'],
        configs['gpuid'])
    exe(train_cmd)
...
def evaluate(configs):
    ...
    translate(
        ...
        gpu=configs['gpu'],
        data_part=configs['data_part'],
        corpus=configs['corpus'],
        max_length=configs['max_length'],
        block_ngram_repeat=configs['block_ngram_repeat']
    )
