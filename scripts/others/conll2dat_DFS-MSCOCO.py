...
class CoNLLTools:
    ...
    def extract_dataframe(self, conll_data, toLower=False):
        ...
        """
        data_frames_strs = [cl for cl in conll_data.split('\n\n')]
        print ('len(data_frames_strs)', len(data_frames_strs))
        ...
        sentences = []
        first = True
        for data_frame_str in data_frames_strs:
            if data_frame_str.strip() != '':
                if toLower:
                    data_frame_str = data_frame_str.lower()
                lines = data_frame_str.split('\n')
                
                
                first_line = lines[0]
                #print(first_line)
                if first_line[0:9] == '# text = ':
                    raw_sent = first_line[9:]
                    lines.pop(0)
                else:
                    continue
                ...
                data_frames = np.array(lines_list).reshape(-1, COL_NUM)
                
                assert data_frames.ndim == 2
                
                # if len(line) <= self.COL_2_PROPS: 
                #     continue
                #print (data_frames)

                if data_frames.ndim == 1:
                    print ('data_frames.ndim == 1', data_frames)

                if data_frames.shape[1] != COL_NUM: 
                    print ('data_frames.shape[1] != COL_NUM', data_frames)
                    continue
                dataframe_list.append(data_frames)
                sentences.append(raw_sent)
                
        print ('len(dataframe)', len(dataframe_list))
    ...
    def old_extract_dataframe(self, conll_data, toLower=False):
        """
        Extract columns from conll files, create an ordered dict of
        ("word", "lemma") pairs and construct sentences for SENNA input.
        ...
        for dl in data_lines:
            if dl.strip() != '':
                ...
                if len(sentence_df) != 0 and not first:
        ...
        for t in dataframe:
            ...
            if t.ndim == 1:
                ...
                num_col = t.shape[1]
                if num_col != COL_NUM:
                    #print ("Add supplement: ", t)
                    supplement = np.zeros((num_line, COL_NUM - num_col))
                    t = np.hstack([t.reshape(-1, num_col), supplement])
...
if __name__ == '__main__':
    ...
    for graph_id, graph_fn in enumerate(graph_fns):
        ...
        # TODO: to fix here@!!! fix the length problem 
        print ('extracting input dataframe ...')
        graph_dfs, sentences = ct.extract_dataframe(graph_data, toLower=False)
