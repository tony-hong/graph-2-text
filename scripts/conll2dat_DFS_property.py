...
class CoNLLTools:
    ...
    def extract_dataframe(self, conll_data, toLower=False):
        ...
        dataframe = np.array([t.reshape(-1, COL_NUM) for t in dataframe])
    ...
    def df_2_dat_DEEP(self, df, output_sent): 
        result = ''
        imgId = 0
        sent_cnt = 0
        isFirst = True
        ...
        for i, line in enumerate(df):
            ...
            if not self.is_ascii(lemma):
                print ("map non-ASCII lemma to <UNKNOWN>: ")
                lemma = '<UNKNOWN>'
...
#             print(properties)
            if properties[0] != '_':
                for property_ent in properties:
                    property_tuple = property_ent.split('=')
#                     print (property_tuple)
                    property_key = property_tuple[0].upper()
                    property_val = property_tuple[1].upper()
                    if property_val == '1':
                        property_val = 'FIRST'
                    if property_val == '3':
                        property_val = 'THIRD'
                    property_item = property_key + ':' + property_val
                    extend_df_list.append([extend_idx, 'PROPERTY', '_', 'PROPERTY', '_', '_', idx, property_item, '_', '_'])
                    extend_idx += 1
            ...
            if pos in {'PROPN', 'NUM'}: 
                ...
                entity_placeholder = 'ENT' + str(head_ID) + str(entityID)
        ...
        extend_df = np.array(extend_df_list)
        
#         print (df)
        if len(extend_df_list) > 0:
            df = np.vstack([df, extend_df])
            extend_lemma_list = extend_df[:, 1]
            delex_lemma_list = np.hstack([delex_lemma_list, extend_lemma_list])
#         print (df)
...
if __name__ == '__main__':
    ...
    prs.add_argument('-s', '--sent',
                     help='Specify directory where target sentence file in CoNLL format are located. ', 
                     required=True)
    ...
    graph_fns = [os.path.join(args.graph, name) for name in os.listdir(args.graph) if re.search(r'conll', name)]
    graph_fns.sort()
    print (graph_fns)
    ...
    sent_fns = [os.path.join(args.sent, name) for name in os.listdir(args.sent) if re.search(r'txt', name)]
    sent_fns.sort()
    print (sent_fns)
    ...
    for graph_id, graph_fn in enumerate(graph_fns):
        ...
        output_name = re.findall(r'_en/(.*)\.conll', graph_fn)[-1]
        ...
        graph_dfs = ct.extract_dataframe(graph_data, toLower=False)

        # token-based 
        print ('reading output sent ...')
        sent_data = ct.gzip_reader(sent_fn).split('\n')
        ...
        for idx_df, graph_df in enumerate(graph_dfs, start=0):
            ...
            input_data, output_sent, entity_lemma_dict, nodes_line, labels_line, node1_line, node2_line = ct.df_2_dat_DEEP(graph_df, sent)
        ...
        input_file_stream += '\n'
        with open(input_file_fn, 'w') as f:
        ...
        nodes_file_stream = '\n'.join(nodes_list)
        nodes_file_stream += '\n'
        with open(nodes_file_fn, 'w') as f:
