...
if __name__ == '__main__':
    ...
    prs.add_argument('-o', '--out', default=os.getcwd(),
                     help='Specify output directory. If not specified, current '
                          'dir is used.',
                     required=True)
    prs.add_argument('-p', '--property', action='store_true',
    ...
    sent_fns = [os.path.join(sent_dir, name) for name in os.listdir(sent_dir) if re.search(r'conll', name)]
    sent_fns.sort()
    print (sent_fns)
    ...
    for graph_id, graph_fn in enumerate(graph_fns):
        ...
        if args.property:
            language = re.findall(r'(en|es|fr)', graph_fn)[-1]
            property_vocab = PROPERTY_MAP[language]
            print ('language', language)

        output_name = re.findall(r'.*/(.*)\.conll', graph_fn)[-1]
        ...
        sent_dfs, sent_data = ct.extract_dataframe(sent_data, toLower=False)
